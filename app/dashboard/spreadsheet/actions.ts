...
export async function bulkUpdateEmployeesAction(
  ...
  if ("error" in session) {
    return { success: false, updatedCount: 0, errors: [{ rowId: "", message: session.error }] }
  }
...
export async function bulkUpdatePayrollsAction(
  ...
  await db.transaction(async (tx) => {
    for (const row of rows) {
      ...
      const payrollPeriodEnd = String(row.payrollPeriodEnd ?? existing.payrollPeriodEnd)
      const dtrCutOffStart = String(row.dtrCutOffStart ?? existing.dtrCutOffStart)
  ...
  await db.transaction(async (tx) => {
    for (const employeeId of input.dirtyEmployeeIds) {
      ...
      if ("error" in schedule) {
        errors.push({
          rowId: resolveScheduleErrorRowId(employeeId, daysResult, schedule.error),
